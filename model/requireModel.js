...
Model.create = function (data, cb) {
    var sql = "INSERT INTO `requirement` (`title`, `description`, `important`) VALUES (?, ?, ?);";
...
Model.get = function (id, field, cd) {
    ...
    if (field == null) {
        ...
        field = field.replace(';', '');
        sql = "SELECT " + field + " as field FROM requirement WHERE id = ?";
    }
...
Model.getAll = function(id, cb){
    var sql = "SELECT * FROM requirement WHERE id = ?";
    db.get().query(sql, [id], function (err, result) {
        if (err) return cb(err);
        cb(null, result[0]);
    });
...
Model.getList = function(offset, limit, cb){
    var sql = "SELECT * FROM requirement WHERE id < ? ORDER BY id DESC LIMIT ?";
...
Model.update = function (id, param, field, cb) {
    ...
    var data = param;
    if (field == null) {
        sql = "UPDATE `requirement` SET `title`= ?, `description`= ?, `important`= ? WHERE `id`= ?";
        data.push(id);
    } else {
        field = field.replace(';', '');        
        sql = "UPDATE `requirement` SET " + field + " = ? WHERE `id` = ?";
        data = [param, id]
    }
    db.get().query(sql, data, function (err, result) {
        if (err) return cb(err);
